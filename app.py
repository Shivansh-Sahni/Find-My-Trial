...
TRIALS_CSV_PATH = os.environ.get("TRIALS_CSV_PATH", "/mnt/data/sample_10000.csv")
...
<style>
:root{
  --primary:#007BFF;--accent:#0056d6;--text:#1a1a1a;--muted:#555;
  --bg:#ffffff;--card:#f9f9f9;--border:#dcdcdc;
}
body{background:var(--bg);color:var(--text);font:16px/1.5 'Inter',system-ui,Arial,sans-serif;margin:0;}
.wrap{max-width:1100px;margin:28px auto;padding:0 20px}
.header{display:flex;align-items:center;gap:16px;margin-bottom:28px}
.logo{width:48px;height:48px;border-radius:10px;background:linear-gradient(135deg,var(--primary),#00bfff)}
h1{font-size:36px;margin:0;color:var(--accent);font-weight:800;letter-spacing:-0.5px}
h2{font-size:24px;margin:10px 0;color:var(--accent);font-weight:700}
.card{
  background:var(--card);border:1px solid var(--border);border-radius:14px;
  padding:22px;margin:16px 0;box-shadow:0 3px 10px rgba(0,0,0,.05);
}
label{font-weight:600;margin:10px 0 6px;display:block}
input[type=text],input[type=number],textarea{
  width:100%;padding:12px 14px;border:1px solid #ccc;border-radius:10px;outline:none;font-size:15px;
}
input:focus,textarea:focus{border-color:var(--primary)}
.controls{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-top:8px}
button{
  background:linear-gradient(135deg,var(--primary),#00bfff);color:white;font-weight:600;
  border:0;border-radius:10px;padding:13px 20px;margin-top:16px;cursor:pointer;transition:all .2s;font-size:16px;
}
button:hover{background:var(--accent)}
.badge{display:inline-block;background:var(--primary);color:white;border-radius:999px;
  padding:4px 12px;margin-right:8px;font-size:13px;}
.title{color:var(--accent);margin:6px 0 2px;font-size:21px;font-weight:600}
...
<div class="card small" style="background:#f2f6ff;border-color:#c8deff;">
  Over <b>500,000+</b> clinical trials loaded. Enter patient details to discover personalized matches.
</div>
...
<form class="card" method="POST" enctype="multipart/form-data">
  ...
  <textarea name="patient_text" rows="7" placeholder="Example: 62 y/o female with metastatic HER2+ breast cancer..."
  >{{patient_text}}</textarea>

  <label>Keywords (optional)</label>
  <input type="text" name="keywords" value="{{keywords}}" placeholder="e.g., breast cancer, HER2, diabetes"/>

  <label>Location (optional)</label>
  <input type="text" name="location_kw" value="{{location_kw}}" placeholder="Detroit, Michigan"/>
  ...
  <div class="controls">
    ...
    <div>
      <label><input type="checkbox" name="recruiting_only" value="1" {% if recruiting_only %}checked{% endif %}/> Recruiting Only</label>
    </div>
  </div>
...
{% if results %}
  <div class="card"><h2>Top {{results|length}} Trial Matches</h2></div>
  {% for r in results %}
    <div class="card">
      <div>
...
def _score(query_text, location_kw, recruiting_only, k):
...
def index():
    ...
    if request.method=="POST":
        ...
        patient_text = request.form.get("patient_text","")
        ...
        query = pipe.build_query_from_patient(patient)
        if keywords:
            query += " " + keywords
        results = _score(query, location_kw, recruiting_only, k)

    return render_template_string(HTML, results=results, patient_text=patient_text,
                                  keywords=keywords, location_kw=location_kw,
                                  k=k, recruiting_only=recruiting_only)
